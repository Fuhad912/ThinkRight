    ...
    signupForm.addEventListener('submit', async (e) => {
        ...
        try {
            ...
            if (result.user && result.needsConfirmation) {
                ...
                const trialCreated = await window.Subscription?.createTrialForNewUser(result.user.id);
                if (trialCreated) {
                    console.log('✓ 24-hour trial created for new user');
                } else {
                    console.warn('⚠️ Failed to create trial, but signup was successful');
                }
