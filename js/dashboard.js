...
async function checkAuthAndLoadDashboard() {
    ...
    try {
        ...
        console.log('⏳ Initializing subscription system for dashboard access check...');
        await window.Subscription?.init();
        
        console.log('🔍 Checking dashboard access...');
        console.log('  Is Admin:', window.Subscription?.isAdmin?.());
        console.log('  Has Paid Subscription:', window.Subscription?.isSubscriptionActive?.());
        console.log('  Can Access Dashboard:', window.Subscription?.canAccessDashboard?.());
        
        if (!window.Subscription?.canAccessDashboard()) {
            console.log('⚠️ User does not have access to dashboard (not admin or paid subscriber)');
            if (!window.Subscription?.isAdmin()) {
                console.log('📢 Showing paywall modal for dashboard...');
                window.Subscription?.showPaywallModal('dashboard');
                // Don't redirect - let user see the paywall modal and click "Back to Home" if they want
                // The modal has a button for this instead of auto-redirect
                return;
            }
        }
