(() => {
  ...
  function initProjection() {
    ...
    const trendEl = document.getElementById("trProjectionTrend");
    ...
    const comparisonCardEl = document.getElementById("homeComparisonCard");
    ...
    loadProjection({ scoreEl, trendEl, badgeEl, rangeEl, stateEl, comparisonCardEl }).catch((err) => {
      ...
      renderHomepageWeeklyComparison(comparisonCardEl, null).catch(() => {});
    });
  }

  async function loadProjection({ scoreEl, trendEl, badgeEl, rangeEl, stateEl, comparisonCardEl }) {
    await waitForAuthInit(1500);

    const user = await safeGetUser();
    await renderHomepageWeeklyComparison(comparisonCardEl, user ? user.id : null);
    ...
    const trend = await fetchProjectionTrendForUser(user.id);
  ...
  async function renderHomepageWeeklyComparison(cardEl, userId) {
    if (!cardEl) return;
    ...
    if (!helper || typeof helper.renderComparisonCard !== "function" || typeof helper.fetchMyWeeklyComparison !== "function") {
      if (messageEl) {
    ...
    if (!userId) {
      helper.renderComparisonCard(cardEl, {
        qualifies: false,
        message: helper.LOCKED_MESSAGE || "Take at least 3 tests this week to unlock comparisons."
      });
      return;
    }
    ...
    try {
      ...
      console.warn("[projection] Weekly comparison unavailable:", error);
      helper.renderComparisonCard(cardEl, {
        qualifies: false,
  ...
  function renderState({
    ...
    applyProjectionTrendChip(trendEl, trend);
  ...
  async function fetchProjectionTrendForUser(userId) {
    const helper = window.ThinkRightProjectionTrend;
    if (!helper || typeof helper.fetchProjectionTrend !== "function") return null;
    try {
      return await helper.fetchProjectionTrend({ userId });
    } catch (e) {
      console.warn("[projection] Trend unavailable:", e);
      return null;
    }
  }

  function applyProjectionTrendChip(trendEl, trend) {
    if (!trendEl) return;
    const helper = window.ThinkRightProjectionTrend;
    if (helper && typeof helper.applyTrendChip === "function") {
      helper.applyTrendChip(trendEl, trend);
      return;
    }
    trendEl.hidden = true;
    trendEl.textContent = "";
    trendEl.className = "tr-projection-trend";
  }
