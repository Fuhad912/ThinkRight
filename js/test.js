...
/**
 * Show/hide loading indicator for question loading
 */
...
async function initTest() {
    try {
        ...
        if (!user) {
            console.warn('User not authenticated. Proceeding in guest mode for debugging (no redirect).');
        } else {
            console.log('✓ User authenticated:', user.email);
        }

        // Initialize subscription and attempt to start test (consumes free test if applicable)
        ...
        try {
            ...
            if (user && window.Subscription && typeof window.Subscription.tryStartTest === 'function') {
                startResult = await window.Subscription.tryStartTest();
            }
        } catch (err) {
            console.error('Subscription init/tryStartTest error:', err);
        ...
        if (!startResult || !startResult.allowed) {
            console.log('❌ User cannot access test:', startResult?.reason || 'unknown');
            if (window.Subscription && typeof window.Subscription.showPaywallModal === 'function') {
                window.Subscription.showPaywallModal('tests');
            }
            // Do not return here — allow overlay or message to show instead of redirecting away
        } else {
