...
async function login(email, password) {
    try {
        ...
        // Ensure user has a baseline subscription row (free, active)
        try {
            await ensureSubscriptionRow(data.user.id);
        } catch (subEnsureErr) {
            console.warn('Could not ensure baseline subscription row:', subEnsureErr);
        }
